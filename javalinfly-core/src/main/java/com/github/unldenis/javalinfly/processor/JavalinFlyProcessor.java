...
public class JavalinFlyProcessor extends AbstractProcessor {
  ...
  public List<String> openApiStatements = new ArrayList<>();

  Set<String> injectorRoles;
  Map<String, ExecutableElement> selectedRoles = new HashMap<>();

  private final OpenApiTranslator openApiTranslator = new OpenApiTranslator();
...
//        }
    ...
    if (!injectors.isEmpty()) {
      ...
      if (injectorElement.getKind() != ElementKind.CLASS) {
        error(injectorElement, "Only classes can be annotated with @%s",
      ...
      boolean implementsInterface = new InterfaceChecker(processingEnv).implementsInterface(
          rolesTypeMirror, "io.javalin.security.RouteRole");

      if (!implementsInterface) {
        error(injectorElement, "Class '%s' does not implement '%s'", rolesTypeMirror.toString(),
    ...
    for (Element elementController : controllers) {
      ...
      if (elementController.getKind() != ElementKind.CLASS) {
        error(elementController, "Only classes can be annotated with @%s",
      ...
      if (varDecl == null) {
        error(annotatedElement, "Class %s missing an empty constructor",
          ...
          if (body != null) {
            ...
            if (body.customType()) {
              if (!typeParameter.equals(String.class.getName())) {
                error(variableElement, "Body parameter '%s' must be a String since is customType",
                    nameParameter);
                return true;
              }
...
//        String packageName = elementUtils.getPackageOf(annotatedElement).getQualifiedName().toString();
      ...
      for (var entry : selectedRoles.entrySet()) {
        if (!injectorRoles.contains(entry.getKey())) {
          error(entry.getValue(), "Role '%s' is missing at @%s", entry.getKey(),
              JavalinFlyInjector.class.getSimpleName());
          return true;
        }
      }
...
//            return true;
  ...
  private void generateClass(Element element) {
    ...
    print(openApiTranslator.asString(new Info(
        "My App",
        "0.1",
        new Contact(
            "API Support",
